...
class Users extends CI_Controller
{
    ...
    function add()
    {
        if (isset($_POST['submit'])) {
            ...
            $this->Model_admins->save($uplodFoto);
            redirect('users');
        } else {
    ...
    function edit()
    {
        if (isset($_POST['submit'])) {
            $uplodFoto = $this->upload_foto_user();
            $this->Model_admins->update($uplodFoto);
            redirect('users');
        } else {
            $id_user       = $this->uri->segment(3);
            $data['user']  = $this->db->get_where('tbl_admin', array('id_user' => $id_user))->row_array();
    ...
    function chek_akses($level_user, $id_menu)
    {
        ...
        $chek = $this->db->get_where('tbl_admin_rule', $data);
        if ($chek->num_rows() > 0) {
            echo "checked";
        }
    }
