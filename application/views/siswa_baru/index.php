<div class="container">

    <div class="row">
        <div class="col-sm-12">
            <div class="header d-block p-2 bg-primary text-white">
                <h3>Informasi</h3>
            </div>
            <div class="d-block p-2 bg-light">
                <div class="porlets-content mt-sm-3">
                    <h1 class="h4 mb-sm-3">Yth Saudara/i.
                        <?php echo strtoupper($tbl_member['nama']) ?>
                    </h1>

                    <p class="mb-0">Selamat, Anda telah berhasil melakukan Pengisian Formulir.</p>
                    <p>Silahkan cetak formulir yang terdapat pada <a href="<?= base_url('member/profile'); ?>" class="text-decoration-none">My Profile</a>, atau klik <a href="<?= base_url('member/editprofile'); ?>" class="text-decoration-none">Edit Profile</a> apabila ingin mengubah data diri.</p>
                    <small>
                        <p class="mt-5">Pertanyaan dan Informasi, silahkan kontak Admin<br><i class="h5 fab fa-whatsapp-square align-middle"></i> +62853 1152 9378</p>
                    </small>
                </div>
            </div>


        </div>
    </div>
</div>
</div>
<!-- End of Main Content -->